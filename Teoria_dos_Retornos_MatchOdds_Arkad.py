...
def TR_MatchOdds():
    for i in ligas:
        ...
        df['H'] = winHPLValues.where(df.FT_Result == 'H', other=losePLValues)
        df['D'] = winDPLValues.where(df.FT_Result == 'D', other=losePLValues)
        df['A'] = winAPLValues.where(df.FT_Result == 'A', other=losePLValues)
...
nome = 'Teoria_dos_Retornos_Match_Odds.csv'
