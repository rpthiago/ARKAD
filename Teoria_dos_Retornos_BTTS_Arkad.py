...
def TR_BTTS():
    for i in ligas:
        ...
        df.loc[((df['BTTS_Yes_No']) == 1), 'BTTS'] = 'BTTS_Sim'
        df.loc[((df['BTTS_Yes_No']) != 1), 'BTTS'] = 'BTTS_Nao'

        df['H'] = winHPLValues.where(df.BTTS == 'BTTS_Sim', other=losePLValues)
        df['A'] = winAPLValues.where(df.BTTS == 'BTTS_Nao', other=losePLValues)
        ...
        for index, row in df2.iterrows():
            if index > 26:
                try:
                    ...
                    if h > a:
                        df3.loc[index, 'R'] = 'BTTS_Sim'
                    else:
                        df3.loc[index, 'R'] = 'BTTS_Não'
                except LookupError:
                    pass
...
nome = 'Teoria_dos_Retornos_BTTS.csv'
